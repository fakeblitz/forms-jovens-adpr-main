...
const schema = z.object({
  ...
  idade: z.coerce
    ...
    .int()
    .min(SITE_CONFIG.minAge, `Idade mínima: ${SITE_CONFIG.minAge} anos`)
    .max(99),
  email: z.string().email("E-mail inválido").max(255),
  telefone: z.string().min(14, "Telefone inválido").max(15),
  saudeObs: z.string().max(1000).optional(),
  // Step 2 - Contato
  contatoEmergencia: z.string().min(3, "Informe o nome do contato").max(100),
  telefoneEmergencia: z.string().min(14, "Telefone inválido").max(15),
  parentesco: z.string().min(2, "Informe o parentesco").max(50),
  // Step 3 - Membresia
  isMembro: z.enum(["sim", "nao"], { required_error: "Selecione uma opção" }),
  igrejaVisitante: z.string().max(200).optional(),
  // Step 4 - Endereço
  endereco: z.string().min(5, "Informe o endereço completo").max(300),
  bairro: z.string().min(2, "Informe o bairro").max(100),
  cep: z.string().min(9, "CEP inválido").max(10),
});
...
const stepFields: Record<number, (keyof FormData)[]> = {
  0: ["nomeCompleto", "idade", "email", "telefone", "saudeObs"],
  1: ["contatoEmergencia", "telefoneEmergencia", "parentesco"],
  2: ["isMembro", "igrejaVisitante"],
...
const RegistrationForm = ({ onSuccess }: RegistrationFormProps) => {
  ...
  const {
    ...
    defaultValues: {
      nomeCompleto: "", email: "", telefone: "", saudeObs: "",
      contatoEmergencia: "", telefoneEmergencia: "", parentesco: "",
      isMembro: undefined, igrejaVisitante: "",
      endereco: "", bairro: "", cep: "",
    },
  });
  ...
  const nextStep = async () => {
    const fields = stepFields[step].filter(f => f !== "saudeObs" && f !== "igrejaVisitante");
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Notify sisters via edge function
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-12 text-center">
        ...
        <p className="text-muted-foreground max-w-sm">
          Agora finalize o pagamento via PIX para garantir sua vaga. Em caso de dúvidas, fale conosco pelo WhatsApp (botão verde no canto da tela).
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col gap-4 w-full max-w-md">
        ...
        <div className="glass-card space-y-3 text-sm">
          ...
          <hr className="border-border" />
          <p><span className="text-muted-foreground">Membro:</span> <span className="text-foreground">{allValues.isMembro === "sim" ? "Sim" : "Não (visitante)"}</span></p>
        ...
        {price !== null && (
          <div className="glass-card flex items-center justify-between">
            ...
            <span className="text-2xl font-display font-bold bg-clip-text text-transparent" style={{ backgroundImage: "var(--gradient-accent)" }}>
        ...
        <div className="flex gap-3">
          <button type="button" onClick={prevStep} className="btn-outline-light flex-1 flex items-center justify-center gap-2 text-sm py-3">
          ...
          >
            {isSubmitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Enviando...</> : "✅ Confirmar Inscrição"}
          </button>
        </div>
      </div>
    );
    ...
    <form onSubmit={(e) => e.preventDefault()} className="flex flex-col gap-4 w-full max-w-md">
      ...
      {step === 0 && (
        <div className="flex flex-col gap-3 animate-fade-in-up">
          ...
          <div>
            ...
            <input {...register("idade")} type="number" className="form-input" placeholder={`Mínimo ${SITE_CONFIG.minAge} anos`} min={SITE_CONFIG.minAge} max={99} />
      ...
      {step === 1 && (
        <div className="flex flex-col gap-3 animate-fade-in-up">
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">Nome do responsavel *</label>
          ...
          <div className="glass-card flex items-start gap-3 p-4 mt-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground leading-relaxed mt-1">
                Suas informações pessoais são mantidas em total privacidade e usadas apenas para comunicações relacionadas ao retiro. Não compartilhamos seus dados com terceiros.
              </p>
            </div>
          </div>
        </div>

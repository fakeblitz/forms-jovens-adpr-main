  ...
  return (
    <div className="py-16 md:py-24 px-4 sm:px-6">
      <div className="max-w-5xl mx-auto text-center">
            ...
            >
              ...
              <div className="flex flex-row items-center justify-center gap-4 sm:gap-10">
                {couple.members.map((member, mi) => (
                  <motion.div
                    key={member.name}
                    className="flex flex-col items-center text-center gap-2 min-w-0 flex-shrink-0"
                    initial={{ opacity: 0, scale: 0.8 }}
                    whileInView={{ opacity: 1, scale: 1 }}
                    viewport={{ once: true }}
                    transition={{ delay: ci * 0.15 + mi * 0.1 + 0.2, duration: 0.5 }}
                  >
                    <div className="w-16 h-16 sm:w-24 sm:h-24 rounded-full overflow-hidden ring-2 ring-primary/40 flex-shrink-0">
                      <img src={member.photo} alt={member.name} className="w-full h-full object-cover" loading="lazy" />
                    </div>
                    <h4 className="font-display font-semibold text-foreground text-xs sm:text-sm truncate max-w-[168px] sm:max-w-none">{member.name}</h4>
        ...
        >
          <h3 className="font-display font-bold text-accent text-sm mb-4 uppercase tracking-wider">Lideres de jovens</h3>
          <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-6 gap-4 sm:gap-6">
            {SITE_CONFIG.leaderSisters.map((sister, si) => (
              ...
              >
                <div className="w-14 h-14 sm:w-20 sm:h-20 rounded-full overflow-hidden ring-2 ring-accent/40 flex-shrink-0">
                  <img src={sister.photo} alt={sister.name} className="w-full h-full object-cover" loading="lazy" />
                </div>
                <h4 className="font-display font-semibold text-foreground text-[10px] sm:text-xs leading-tight">{sister.name}</h4>
                <span className="text-[9px] sm:text-xs text-accent font-medium">{sister.role}</span>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 sm:gap-8">
            {SITE_CONFIG.leaderAuxiliares.map((sister, si) => (
              <motion.div
                key={sister.name}
                className="flex flex-col items-center text-center gap-2"
                initial={{ opacity: 0, y: 20 }}
              ...
              >
                <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full overflow-hidden ring-2 ring-accent/40 flex-shrink-0">
                  <img src={sister.photo} alt={sister.name} className="w-full h-full object-cover" loading="lazy" />
                </div>
